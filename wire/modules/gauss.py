...
class GaussLayer(nn.Module):
    ...
    def __init__(self,opt, in_features, out_features,rank_k ,fre,is_first=False):
        ...
        if is_first:
            ...
            self.fix = nn.Parameter(torch.randn(out_features,in_features),requires_grad=False)
            nn.init.kaiming_uniform_(self.fix,a=math.sqrt(5))
    ...
    def forward(self, input):
        ...
        if self.is_first:
            ...
            # return self.gaussian(F.linear(input,x,self.bias))
            return self.gaussian(F.linear(input,x/2+self.fix,self.bias))
