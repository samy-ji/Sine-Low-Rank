...
from .tuners import LoraConfig, PrefixTuningConfig, PromptEncoderConfig, PromptTuningConfig, BottleneckConfig, DoraConfig, sinLoraConfig,sinDoraConfig
...
PEFT_TYPE_TO_CONFIG_MAPPING = {
    ...
    "DORA": DoraConfig,
    "SINLORA": sinLoraConfig,
    "SINDORA": sinDoraConfig,
}
...
def get_peft_model(model, peft_config):
    ...
    if peft_config.task_type not in MODEL_TYPE_TO_PEFT_MODEL_MAPPING.keys():
        if peft_config.peft_type == "LORA" or "SINLORA":
            peft_config = _prepare_lora_config(peft_config, model_config)
            return PeftModel(model, peft_config)
        elif peft_config.peftype == "DORA" or "SINDORA":
    ...
    if not isinstance(peft_config, PromptLearningConfig):
        if peft_config.peft_type == "BOTTLENECK":
            peft_config = _prepare_bottleneck_config(peft_config, model_config)
        elif peft_config.peft_type == "LORA" or "SINLORA":
            peft_config = _prepare_lora_config(peft_config, model_config)
        elif peft_config.peft_type == "DORA" or "SINDORA":
            peft_config = _prepare_dora_config(peft_config, model_config)
    else:
