...
@dataclass
class sinDoraConfig(PeftConfig):
    """
    ...
    learnable: bool = field(default=False, metadata={"help": "Whether to learn the sine frequency"})
    ...
    def __post_init__(self):
        self.peft_type = PeftType.SINDORA


class sinDoraModel(torch.nn.Module):
    """
    ...
    **Attributes**:
    ...
    def _find_and_replace(self):
        ...
        kwargs = {
            ...
            "learnable": self.peft_config.learnable,
        }
    ...
    def _set_adapter_layers(self, enabled=True):
        for module in self.model.modules():
            if isinstance(module, sinLoraLayer):
...
def mark_only_lora_as_trainable(model: nn.Module, bias: str = "none") -> None:
    ...
    if bias == "none":
        ...
        for m in model.modules():
            if isinstance(m, sinLoraLayer) and hasattr(m, "bias") and m.bias is not None:
...
class Linear(nn.Linear, sinLoraLayer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        sinLoraLayer.__init__(self, r=r, lora_alpha=lora_alpha, lora_dropout=lora_dropout, merge_weights=merge_weights,freq=freq,s=s,learnable=learnable)
        ...
        if self.learnable:
            self.freq = nn.Parameter(torch.tensor(self.freq, dtype=torch.float32))
            self.s = nn.Parameter(torch.tensor(math.sqrt(in_features), dtype=torch.float32))
        else:
        ...
        if not mode and self.merge_weights and not self.merged:
            ...
            if self.Wdecompose:
                ...
                if self.r > 0:
                    ...
                    new_weight_v = self.weight + transpose(torch.sin(self.freq*self.lora_B.weight @ self.lora_A.weight)/self.s, fan_in_fan_out=self.fan_in_fan_out) * self.scaling
...
class MergedLinear(nn.Linear, sinLoraLayer):
    # Lora implemented in a dense layer
    def __init__(
        self,
        in_features: int,
...
if is_bnb_available():
    ...
    class Linear8bitLt(bnb.nn.Linear8bitLt, sinLoraLayer):
        # Lora implemented in a dense layer
        def __init__(
            self,
            in_features,
    ...
    class MergedLinear8bitLt(bnb.nn.Linear8bitLt, sinLoraLayer):
        # Lora implemented in a dense layer
        def __init__(
            self,
            in_features: int,
